...
class Mytree{
    ...
    function radd($treedata){
        if(!is_array($treedata)){
            var_dump($treedata);
        }
        if (!isset($treedata[$this->childrenKey])) {
            $this->nodeacc[$treedata[$this->pk]] = 1;
            return 1;
        }
        $sum=0;
        foreach ($treedata[$this->childrenKey] as $key) {
            $sum+=$this->radd($key);
        }
        $sum+=1;
        $sum*=2;
        $this->nodeacc[$treedata[$this->pk]] = $sum;
        return $sum;
    }
}